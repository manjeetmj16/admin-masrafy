...
const Ads = (props) => {
    ...
    const formFields = [
        { label: 'Heading', name: 'heading', type: 'text', placeholder: 'Banking Start Here', required: true },
        { label: 'Paragraph', name: 'paragraphtext', type: 'textarea', placeholder: 'Paragraph here', required: true, minLength: 10 },
        { label: 'Primary Button Text', name: 'primarybuttontext', type: 'text', placeholder: 'Open account', required: true },
        { label: 'Secondary Button Text', name: 'secondarybuttontext', type: 'text', placeholder: 'Offerings', required: true },
    ];
                                ...
                                {formFields.map((field, index) => (
                                    <div className={`col-lg-6`} key={index}>
                                        <div className='mas_input_box'>
                                            ...
                                            {field.type === 'textarea' ? (
                                                <textarea
                                                    defaultValue={props?.AdsData?.[field.name]}
                                                    placeholder={field.placeholder}
                                                    {...register(field.name, { 
                                                        required: field.required, 
                                                        minLength: field.minLength && {
                                                            value: field.minLength,
                                                            message: `message must be at least ${field.minLength} characters long`,
                                                        } 
                                                    })}
                                                />
                                            ) : (
                                            ...
                                            {errors[field.name] && <p>{errors[field.name]?.message || 'Required'}</p>}
                                        </div>
                                    </div>
                                ))}
